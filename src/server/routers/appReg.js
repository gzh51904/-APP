...
.get((req,res)=>{
    ...
    query('select * from appReg').then(function(data){
...
.post((req,res)=>{
    ...
    let sql = `insert into users(${names}) values(${values})`;
...
.patch((req,res)=>{
    ...
    let sql = `update admin set ${opt} where name=${id}`;
...
.get((req,res)=>{
    ...
    // connection.connect();
    // // 查询数据库
    // connection.query(`select * from appReg where id=${id}`, function (error, results, fields) {
    //     if (error) throw error;
    //    // 关闭连接,释放资源
    //     connection.end();
    //     console.log(results)
    ...
    // pool.query(`select * from appReg where id=${id}`, function(error, data){
    //     res.send(formatData({data:data[0]}))
    // });

    // 查询数据库是否存在该用户名
    query(`select * from users where username=${id}`).then(data => {
        console.log("11:",data[0].name,data[0].psw);
        if(data.length>0){
            res.send([{
                state1 : "Well Done!"
            },{
...
.delete((req,res)=>{
    ...
    let sql = `delete from appReg where id=${id}`;
